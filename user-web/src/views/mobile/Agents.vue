...
async function load() { const res = await api.get('/tenant/employees'); if (res.code === 0) employees.value = res.data }
async function createEmployee() {
  if (!createForm.value.username || !createForm.value.displayName || !createForm.value.password) { alert('请填写完整信息'); return }
  const res = await api.post('/tenant/employees', createForm.value)
  if (res.code === 0) { showCreate.value = false; createForm.value = { username: '', displayName: '', password: 'employee123', role: 'agent' }; await load() }
  else alert(res.message)
}
function openEdit(employee) { editTarget.value = employee; editForm.value = { username: employee.username, displayName: employee.displayName, role: employee.role } }
async function saveEmployee() {
  const res = await api.patch(`/tenant/employees/${editTarget.value._id}`, editForm.value)
  if (res.code === 0) { editTarget.value = null; await load() } else alert(res.message)
}
async function toggleStatus(employee) { await api.patch(`/tenant/employees/${employee._id}`, { status: employee.status === 'active' ? 'disabled' : 'active' }); await load() }
async function deleteEmployee() { if (!deleteTarget.value) return; await api.delete(`/tenant/employees/${deleteTarget.value._id}`); deleteTarget.value = null; await load() }
async function doReset() {
  ...
  if (res.code === 0) { alert('已重置'); resetTarget.value = null; resetPwd.value = '' } else alert(res.message)
}
...
<template>
  <div class="page-content">
    ...
    <div class="employee-list">
      <article v-for="employee in employees" :key="employee._id" class="employee-card">
        <div class="employee-main"><strong>{{ employee.displayName }}</strong><span>@{{ employee.username }}</span></div>
        <div class="employee-meta">
          <span class="tag" :class="{ 'tag-blue': employee.role === 'owner', 'tag-green': employee.role === 'admin', 'tag-gray': employee.role === 'agent' }">{{ { owner: '所有者', admin: '管理员', agent: '员工' }[employee.role] }}</span>
          <span class="tag" :class="employee.status === 'active' ? 'tag-green' : 'tag-red'">{{ employee.status === 'active' ? '正常' : '已禁用' }}</span>
        </div>
        <small>最后登录：{{ employee.lastLoginAt ? new Date(employee.lastLoginAt).toLocaleString() : '—' }}</small>
        <div v-if="employee.role !== 'owner'" class="employee-actions">
    ...
    <div v-if="showCreate" class="modal-overlay" @click.self="showCreate = false"><div class="modal-box">
      <h3>新增员工</h3><div class="form-group"><label>用户名</label><input v-model="createForm.username" /></div><div class="form-group"><label>显示名</label><input v-model="createForm.displayName" /></div><div class="form-group"><label>初始密码（至少6位）</label><input type="password" v-model="createForm.password" /></div><div class="form-group"><label>角色</label><select v-model="createForm.role"><option value="agent">员工</option><option value="admin">管理员</option></select></div><div class="modal-footer"><button class="btn-ghost" @click="showCreate = false">取消</button><button class="btn-primary" @click="createEmployee">创建</button></div>
    </div></div>
...
.page-title { margin-bottom: 14px; font-size: 17px; }
.employee-list { display: grid; gap: 12px; }
.employee-card { display: grid; gap: 10px; padding: 16px; border: 1px solid #e2e8f0; border-radius: 14px; background: #fff; }
.employee-main { display: flex; justify-content: space-between; gap: 12px; }
.employee-main span, small { color: #64748b; }
.employee-meta, .employee-actions { display: flex; gap: 8px; flex-wrap: wrap; }
.modal-box { width: calc(100vw - 24px); max-height: calc(100dvh - 32px); overflow-y: auto; }
</style>
