...
function handleMessageScroll(event) {
  ...
  // #region debug-point B-D:desktop-scroll-event
  if (initialBottomSession) reportDebugEvent('B-D', 'desktop/ChatPanel.vue:handleMessageScroll', 'scroll observed during correction', { phase: 'scroll', conversationId: String(props.conversationId || '').slice(-6), generation: messageGeneration, targetPresent: Boolean(props.targetMessageId), inputTrusted: Boolean(event?.isTrusted), scrollElementIdentity: container === msgContainer.value ? 'msgContainer' : 'eventCurrentTarget', scrollTop: Math.round(container?.scrollTop || 0), clientHeight: Math.round(container?.clientHeight || 0), scrollHeight: Math.round(container?.scrollHeight || 0), isNearBottom: isNearBottom() })
  // #endregion
...
async function sendMsg() {
  ...
  } catch (e) { if (epoch === conversationEpoch) { input.value = text; alert(e?.message || '发送失败') } }
...
async function handleUpload(ev) {
  ...
  try {
    ...
    fetch(`/api/__debug_agent_upload_zero/start?surface=desktop&size=${encodeURIComponent(file.size)}&type=${encodeURIComponent(selectedType)}`, { credentials: 'same-origin' }).catch(() => {})
    // #endregion
    const res = await api.post(`/tenant/conversations/${conversationId}/attachments`, fd, {
      ...
      onUploadProgress: event => {
        ...
        fetch(`/api/__debug_agent_upload_zero/progress?surface=desktop&loaded=${encodeURIComponent(event.loaded ?? '')}&total=${encodeURIComponent(event.total ?? '')}&computable=${encodeURIComponent(event.lengthComputable ?? '')}&progress=${encodeURIComponent(event.progress ?? '')}&pending=${pendingExists ? 1 : 0}`, { credentials: 'same-origin' }).catch(() => {})
        // #endregion
...
async function scrollToLatest(correctLayout = false) {
  const generation = messageGeneration
  const conversationId = props.conversationId
  // #region debug-point A-C:desktop-scroll-start
  reportDebugEvent('A-C', 'desktop/ChatPanel.vue:scrollToLatest', 'latest scroll requested', { phase: 'start', conversationId: String(conversationId || '').slice(-6), generation, targetPresent: Boolean(props.targetMessageId), correctLayout, scrollElementIdentity: msgContainer.value ? 'msgContainer' : 'missing' })
  // #endregion
  await nextTick()
  ...
  if (generation !== messageGeneration || String(conversationId) !== String(props.conversationId) || positionMode.value === 'search' || !following.value) {
    // #region debug-point C:desktop-scroll-skipped
    reportDebugEvent('C', 'desktop/ChatPanel.vue:scrollToLatest', 'latest scroll skipped', { phase: 'cancel', reason: generation !== messageGeneration ? 'generation-changed' : String(conversationId) !== String(props.conversationId) ? 'conversation-changed' : 'target-present', conversationId: String(conversationId || '').slice(-6), generation, currentGeneration: messageGeneration, targetPresent: Boolean(props.targetMessageId) })
    // #endregion
    return
  }
  const container = msgContainer.value
...
function isDownloaded(msg) {
  ...
  return Boolean(msg?.attachmentId && wasAttachmentDownloaded(localStorage, cacheScope, msg.attachmentId))
}
      ...
      <div class="cp-body" ref="msgContainer" @scroll="handleMessageScroll" tabindex="0" @wheel.passive="position.input" @touchstart.passive="position.input" @touchmove.passive="position.input" @keydown="position.input" @pointerdown="position.input">
        ...
        <template v-for="(msg, idx) in messages" :key="msg._id">
          ...
          <div v-else class="cp-bubble-row" :data-message-id="msg._id" :class="msg.senderType === 'customer' ? 'is-left' : 'is-right'">
            ...
            <template v-else>
              <div class="cp-bubble-wrap">
                <div class="cp-bubble" :class="{ 'bare-media': ['image', 'video'].includes(msg.messageType), 'cp-media-message-bubble': ['image', 'video', 'file'].includes(msg.messageType) && (msg.attachmentId || msg.attachmentUrl), 'cp-menu-active': contextMenu?.msg === msg }" @contextmenu="showContextMenu($event, msg)" @touchstart="startLongPress($event, msg)" @touchend="finishLongPress" @touchcancel="cancelLongPress" @touchmove="moveLongPress" @click.capture="handleBubbleClick">
                  <template v-if="msg.recalledAt"><span class="cp-recalled">消息已撤回</span></template>
                  <template v-else-if="attachmentExpired(msg)"><span class="chat-media-expired">该文件已过期并自动清理</span><div v-if="['image', 'video'].includes(msg.messageType) && imageCaption(msg)" class="cp-image-caption">{{ imageCaption(msg) }}</div></template>
                  <template v-else-if="msg.messageType === 'image'">
                    <button type="button" class="chat-media" :disabled="Boolean(msg.uploadPhase)" :aria-label="msg.uploadPhase ? '附件尚未发送' : '查看原图'" @click="openPreview(msg)">
...
.chat-media-placeholder { position: absolute; inset: 0; display: grid; place-items: center; padding: 12px 8px 32px; color: #e2e8f0; font-size: 12px; }
