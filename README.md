...
一键安装完成后会把管理命令注册到 `/usr/local/bin/ymkf`。在服务器任意目录执行：
...
- **安装 / 升级**：未安装时启动一键安装程序；已安装时下载最新 `KF.tar.gz`、保留 `.env` 和 Docker 数据卷、更新程序文件并重新构建容器。
- **重置管理员密码**：按管理员账号查找平台管理员，使用 bcrypt 写入新密码，同时清除登录失败次数和临时锁定状态；密码不会写入日志。
- **卸载**：可选择仅移除容器并保留数据，或输入 `DELETE` 二次确认后永久删除容器、数据卷、上传文件和安装目录。
- **重置端口**：重新配置 API、平台管理端、租户客服端和客户聊天端口，同步更新 `.env` 中的 CORS 与客户端公开地址并重建容器。
- **退出**：关闭菜单，不改变服务状态。

安装目录记录在 `/etc/yimeng-kf.path`。如使用防火墙、云安全组或反向代理，重置端口后需同步修改其规则。

如升级包使用自定义地址，可在启动菜单时传入：

```bash
sudo env KF_PACKAGE_URL=https://example.com/KF.tar.gz ymkf
```
